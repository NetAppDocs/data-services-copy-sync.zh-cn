...
summary: 创建新的数据代理时，请选择 On-Prem Data Broker 选项在本地 Linux 主机或云中的现有 Linux 主机上安装数据代理软件。  NetApp Copy and Sync 将指导您完成安装过程，但此页面上重复了要求和步骤，以帮助您做好安装准备。 
---
= 在 Linux 主机上安装用于NetApp复制和同步的数据代理
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
为NetApp Copy and Sync 创建新的数据代理组时，请选择 On-Prem Data Broker 选项在本地 Linux 主机或云中的现有 Linux 主机上安装数据代理软件。  NetApp Copy and Sync 将指导您完成安装过程，但此页面上重复了要求和步骤，以帮助您做好安装准备。
...
+
URI:: 复制和同步会在说明页面上显示安装文件的 URI，当您按照提示部署 On-Prem 数据代理时，该 URI 会加载。该 URI 不会在此重复，因为链接是动态生成的并且只能使用一次。<<安装数据代理,按照以下步骤从复制和同步中获取 URI>> 。
