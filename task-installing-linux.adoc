...
+
必须向 Red Hat Subscription Management 注册 Red Hat Enterprise Linux 系统。如果未注册，则系统无法在安装过程中访问存储库以更新所需的第三方软件。
...
. 选择*创建新同步*。
. 在 *Define Sync Relationship* 页面上，选择源和目标，然后选择 *Continue*。
+
完成这些步骤，直到到达*数据经纪人组*页面。

. 在 *Data Broker Group* 页面上，选择 *Create Data Broker*，然后选择 *On-Prem Data Broker*。
+
...
. 输入数据代理的名称，然后选择 *Continue*。
+
说明页面加载后，请按照包含唯一链接的说明下载安装程序。

. 在说明页面上：
+
.. 选择启用对 *AWS*、*Google Cloud* 或两者的访问。
