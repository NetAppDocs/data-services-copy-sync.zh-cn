...
NOTE: Copy Sync 会复制 SMB ACL（权限），但不会复制文件或文件夹的所有权。所有权属性不包含在 SMB ACL 复制操作中。如果在 SMB 共享之间复制数据时需要保留所有权，请使用 `robocopy`手动复制安全信息。例如， `/copyall`标记副本 ACL、所有者和审计数据。
