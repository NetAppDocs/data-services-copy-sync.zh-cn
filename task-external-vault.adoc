...
summary: 当您创建需要 Amazon S3、Azure 或 Google Cloud 凭据的同步关系时，您需要通过NetApp Copy and Sync用户界面或 API 指定这些凭据。另一种方法是设置数据代理以直接从外部 HashiCorp Vault 访问凭据。 
---
= 设置数据代理组以在NetApp Copy and Sync中使用外部 HashiCorp Vault
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
当您创建需要 Amazon S3、Azure 或 Google Cloud 凭据的同步关系时，您需要通过NetApp Copy and Sync用户界面或 API 指定这些凭据。另一种方法是设置数据代理组以直接从外部 HashiCorp Vault 访问凭据（或_秘密_）。
...
--
** 类型：字符串
** 您的命名空间（如果需要，请输入 X-Vault-Namespace 标头）
...
....
* 要获取用户令牌和您的NetApp Console帐户 ID，link:api-sync.html["请参阅文档中的此页面"] 。
